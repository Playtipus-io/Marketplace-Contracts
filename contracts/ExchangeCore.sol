...
import "./generic/ReentrancyGuarded.sol";
...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    /* minter of nfts */
    ERC721Controller public erc721Controller;
    ...
    struct Order {
        ...
        uint tokenId;
        /* uri in case of lazy minting, empty if already minted */
        bytes uri;
    ...
    /**
     * @dev Change erc721 controller (owner only)
     * @param newERC721Controller New fee to set in basis points
     */
    function changeERC721Controller(ERC721Controller newERC721Controller)
        public
        onlyOwner
    {
        erc721Controller = newERC721Controller;
    }

    /**
     * @dev Change fee paid to creators from secondary sales (owner only)
     * @param newCreatorFee New fee to set in basis points
     */
    function changeCreatorFee(uint newCreatorFee)
        public
        onlyOwner
    {
     ...
     */
    function sizeOfOrder(Order memory order)
        internal
        pure
        returns (uint)
    {
        return (0x14 * 6) + (0x20 * 5) + order.uri.length;
    }
        ...
        returns (bytes32 hash)
    {
        ...
        index = ArrayUtils.unsafeWriteBytes(index, order.uri);
        ...
        returns (bool)
    {
        return (
            ...
            /* Match trade action */
            ArrayUtils.arrayEq(buy.uri,sell.uri) &&
        ...
        reentrancyGuard
    {
        ...
        PlaytipusERC721 target = PlaytipusERC721(sell.target);
        address creator = target.creatorOf(tokenId);

        /* Execute funds transfer and pay fees. */
        uint price = executeFundsTransfer(buy, sell, creator);
     ...
     * @param sell order
     */
    function executeFundsTransfer(Order memory buy, Order memory sell, address creator)
        internal
        returns (uint)
    {
        ...
        uint sellerFee = SafeMath.sub(receiveAmount, SafeMath.add(calculatedProtocolFee, calculatedCreatorFee));
        ...
        if (buy.tokenAddress != address(0)) {
            ...
            /* Execute specified call to transfer creator fee tokens through proxy. */
            bytes memory creatorFeeCallData = tokensTransferCalldata(buy.buyer, creator,
                                                                      calculatedCreatorFee);
            require(proxy.proxy(sell.tokenAddress, AuthenticatedProxy.HowToCall.Call, creatorFeeCallData));
        ...
        returns(uint)
    {
        ...
        if (sell.uri.length != 0) {
            tokenId = erc721Controller.safeMint(PlaytipusERC721(sell.target), sell.seller, string(sell.uri));
        }
